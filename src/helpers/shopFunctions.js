...
export const createProductElement = ({ id, title, thumbnail, price }) => {
  ...
  cartButton.addEventListener('click', async () => {
    const fetched = async () => {
      const resultFetch = await fetchProduct(id);
      return resultFetch;
    };
    saveCartID(id);
    const thingsToCart = await fetched();
    const {
      id: identity,
      title: titulo,
      price: cost,
    ...
    const productToBuy = createCartProductElement({
      id,
      title,
      price,
      pictures,
    ...
    console.log(identity, titulo, cost);
    const sectionToBuy = document.querySelector('.cart__products');
    sectionToBuy.appendChild(productToBuy);
  });
